    ...
    <div class="d-flex float-right mr-3 mt-3">
        <form action="#" method="GET">
            <div class="input-group class=" float-left">
                <input type="text" class="form-control bg-dark text-white border border-white border-top-0 border-left-0 border-right-0" name="search" placeholder="Enter name to search" value="{{ old('search') }}">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    @foreach(Auth::user()->roles as $role)
                    @if($role->name == 'Admin')
                    <button type="submit" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#DeleteOrderModal{{$order->order_id}}">
